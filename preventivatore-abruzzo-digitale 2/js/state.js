...
const State = {
  ...
  defaultUsers: [
    ...
    { username: 'mattia',    password: 'mattia2026', name: 'Mattia',    role: 'operator' },
  ],
  ...
  async login(username, password) {
    ...
    if (!users || users.length === 0) {
      ...
      // Aggiungi utenti default MANCANTI (idempotente). Serve quando aggiungiamo
      // nuovi utenti default in update successivi (es. Mattia introdotto nella
      // sessione di maggio 2026): senza questo, il browser di chi ha già
      // l'IndexedDB seedato non vede mai i nuovi utenti.
      const existingUsernames = new Set(users.map(u => String(u.username).toLowerCase()));
      const missing = this.defaultUsers.filter(d => !existingUsernames.has(d.username.toLowerCase()));
      if (missing.length) {
        console.log('[Login] Aggiungo utenti default mancanti:', missing.map(u => u.username).join(', '));
        try {
          for (const u of missing) await DB.put('users', u);
          users = await DB.all('users');
        } catch (e) {
          console.warn('[Login] Add utenti mancanti fallito:', e);
    ...
    // Fallback al backend: se IndexedDB non ha l'utente, prova /auth/login.
    // Serve per utenti creati dall'admin via UI Utenti che non sono ancora
    // in IndexedDB di chi prova a loggare.
      ...
      try {
        ...
        if (base) {
          ...
          if (r.ok) {
            ...
            const backendUser = { ...j.user, password: cleanPass };
            // Salvo l'utente in IndexedDB locale per le sessioni future
            try { await DB.put('users', backendUser); } catch {}
            this.currentUser = backendUser;
            try { localStorage.setItem('ad_session', JSON.stringify({ username: backendUser.username, ts: Date.now() })); } catch {}
            console.log('[Login] OK via backend:', backendUser.name);
            return true;
          }
        }
      } catch (e) { console.warn('[Login] backend fallback fallito:', e.message); }
    }
