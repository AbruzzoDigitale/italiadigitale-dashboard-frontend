  ...
  areas: [
    {
      ...
      sections: [
        {
          id: 'social-pacchetti',
          name: 'Pacchetti Social Media (all-inclusive)',
          icon: '⭐',
          requiredHint: 'Pacchetti pronti: ognuno include strategia + post + stories + reel + 1 campagna ADS + setup. Se aggiungi un pacchetto, la sezione Gestione & Strategia non è obbligatoria.',
          boxes: [
            { id: 'pkg-social-visibility', label: 'Pacchetto Social Media Visibilità', familyLabel: 'Pacchetto Social Media Visibilità', price: 650, period: 'monthly', desc: '4-6 post · 1 reel · 4-6 stories · 1 campagna Meta ADS · setup Meta BM + GMB · shooting foto/video' },
            { id: 'pkg-social-growth',     label: 'Pacchetto Social Media Crescita',    familyLabel: 'Pacchetto Social Media Crescita',   price: 850, period: 'monthly', desc: '6-8 post · 1-2 reel · 6-8 stories · 2 campagne Meta ADS · setup completo · shooting · 1 reel brand extra' },
            { id: 'pkg-social-evolution',  label: 'Pacchetto Social Media Evoluzione', familyLabel: 'Pacchetto Social Media Evoluzione',price: 1200,period: 'monthly', desc: '6-8 post · 3-4 reel · 8-10 stories · 3 campagne Meta ADS · setup completo · shooting · 1 reel brand extra' },
          ],
        },
        {
          id: 'social-strategia',
    ...
    },
    {
      ...
      name: 'Grafica',
      icon: '🎨',
      desc: 'Servizi grafici dell\'agenzia. La lista si popola automaticamente sincronizzando il listino da Fatture in Cloud (categoria "Grafica"). Per aggiornare: Integrazioni → Fatture in Cloud → Sincronizza listino.',
      sections: [
        {
          id: 'grafica-listino',
          name: 'Servizi grafici (da listino FiC)',
          icon: '🖌',
          autoPopulateFromCategory: 'Grafica',   // marker per populateFromListino()
          boxes: [],
        },
      ],
    },
...
const Configurator = {
  ...
  /** Popola le sezioni con autoPopulateFromCategory leggendo i prodotti
      FiC (es. tab Grafica). Mantiene gli ID stabili: <area>-<ficId>. */
  populateFromListino(ficProducts) {
    if (!Array.isArray(ficProducts)) return 0;
    let n = 0;
    for (const area of ConfiguratorData.areas) {
      for (const sec of area.sections) {
        if (!sec.autoPopulateFromCategory) continue;
        const cat = sec.autoPopulateFromCategory;
        const matching = ficProducts.filter(p => {
          const c = p.category && (p.category.name || p.category);
          return c && String(c).toLowerCase() === String(cat).toLowerCase();
        });
        sec.boxes = matching.map(p => ({
          id: `${area.id}-fic-${p.id}`,
          label: p.name,
          desc: p.description || '',
          price: Number(p.net_price) || 0,
          period: /mensil|mese/i.test(p.measure || '') ? 'monthly' : 'oneoff',
          ficProductId: p.id,
        }));
        n += sec.boxes.length;
      }
