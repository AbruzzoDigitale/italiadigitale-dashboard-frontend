...
const Brand = {
  ...
  async applyLogos() {
    ...
    // Login: priorità al logo DEDICATO 'login_logo' (caricato da
    // Personalizza > Schermata di login), poi fallback al lockup
    // verticale scuro (o pittogramma).
    const loginLogoCustom = await DB.getSetting('login_logo', null);
    this._setLogo('loginLogo', loginLogoCustom || all.vert_dark || all.mark_dark || all.horiz_dark);
