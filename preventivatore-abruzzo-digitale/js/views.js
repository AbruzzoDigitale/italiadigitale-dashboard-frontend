...
const Views = {
  ...
  async renderQuoteEditor() {
    ...
    // Title
    document.getElementById('preventivoTitle').textContent =
      q.id ? `Preventivo ${q.number}` : `Nuovo preventivo · ${q.number}`;
  ...
  renderLines() {
    ...
    body.innerHTML = q.lines.map((l, i) => {
      const included = l && (l.included === true || l.lineKind === 'included_line');
      return `
      <div class="line-item ${included ? 'line-item--included' : ''}">
        <div>
          <div class="line-item__cat">${l.category || ''}${included ? ' · incluso nel bundle' : ''}</div>
        ...
        ${included
          ? `<button class="remove is-locked" title="Voce inclusa nel bundle, non rimovibile" disabled>
  ...
  _removeLine(i) {
    const line = State.currentQuote.lines[i];
    if (line && (line.included === true || line.lineKind === 'included_line')) return;
    State.currentQuote.lines.splice(i, 1);
    this.renderLines();
    this.recalcQuote();
  },

  _buildManualPayload(q) {
    return {
      ...
      discount_eur: q.discountEur || 0,
      fic_id: q.ficId || null,
      lines: q.lines || [],
      configurator: q.configurator || null,
    };
  },
  ...
  async _saveQuote() {
    ...
    const clientIdValue = parseInt(document.getElementById('quoteClient').value) || null;
    const notesValue = document.getElementById('quoteNotes').value || null;
    const discountPctValue = parseFloat(document.getElementById('quoteGlobalDiscPct').value) || 0;
    const discountEurValue = parseFloat(document.getElementById('quoteGlobalDiscEur').value) || 0;
    const dateValue = document.getElementById('quoteDate').value || null;

    q.clientId = clientIdValue;
    q.notes = notesValue || '';
    q.discountPct = discountPctValue;
    q.discountEur = discountEurValue;
    q.date = dateValue || q.date;

    const hasPreviewPayload = !!q._previewPayload;
    if (hasPreviewPayload) {
      const payload = {
        ...q._previewPayload,
        client_id: clientIdValue,
        notes: notesValue,
        discount_pct: discountPctValue,
        discount_eur: discountEurValue,
        date: dateValue,
      };

      let saved;
      try {
        saved = await Sync._api('POST', '/api/v1/quotes/from-configurator', payload);
      } catch (e) {
        // Se la sync non e disponibile, mantieni comportamento locale.
      ...
      if (saved && typeof saved === 'object') {
        q.id = saved.id || q.id;
        q.number = saved.number || q.number;
        q.lines = Array.isArray(saved.lines) ? saved.lines : q.lines;
      }
      delete q._previewPayload;
      delete q._previewTotals;
      await State.saveQuote();
      return saved || q;
    }

    // Flusso manuale / legacy
    const manualPayload = this._buildManualPayload(q);
    try {
      await Sync._api('POST', '/api/v1/quotes', manualPayload);
    } catch (e) {
      // Ignora errore backend e salva locale come sempre.
    }
